...
void Gfx::setVideoMode()
{
	...
	if (is_classic)
	{
		screen = SDL_SetVideoMode(320, 200, has_colors ? 16 : 8, SDL_SWSURFACE);
	}
	else
	{
...
{
#ifdef NSPIRE
	FILE* exe = openFile(joinPath(lieroDataRoot, "palette.dat.tns"));
#else
...
{
#ifdef NSPIRE
	FILE* exe = openFile(joinPath(lieroDataRoot, "menus.dat.tns"));
#else
...
{
#ifdef NSPIRE
	FILE* exe = openFile(joinPath(lieroDataRoot, "bonus.dat.tns")); 
#else
...
#ifdef NSPIRE
	#if defined(HOME_DIR)
		std::string levelPath = joinPath(lieroConfigRoot, game.settings.levelFile + ".lev.tns");
	#else
		std::string levelPath = joinPath(lieroDataRoot, game.settings.levelFile + ".lev.tns");
	#endif
#else
	#if defined(HOME_DIR)
