...
void setLieroDataRoot(std::string const& path)
{
	...
	/*lieroDataRoot = path;*/
	lieroDataRoot = "/documents/liero/data/";
	
#ifdef NSPIRE
	lieroCHR = joinPath(lieroDataRoot, "LIERO.CHR.TNS");
	lieroSND = joinPath(lieroDataRoot, "LIERO.SND.TNS");
	lieroOPT = joinPath(lieroDataRoot, "LIERO.OPT.TNS");
#else
